...
// ⬇️ ZOMBIE: imports añadidos
import Enemy from '../entities/Enemy.js'
import { loadZombie, makeZombieInstance } from '../loaders/ZombieLoader.js'
...
export default class Experience {
  constructor(_canvas) {
    ...
    // ⬇️ ZOMBIE: lista para enemigos y arranque de enemigos
    this.enemies = []
    this._initEnemies() // carga GLB y spawnea cuando el robot esté listo
  ...
  // ⬇️ ZOMBIE: carga modelo y crea enemigos
  async _initEnemies() {
    try {
      const { base: zombieBase, animations: zombieAnims } = await loadZombie()
      // Espera a que el robot (playerRef) exista y tenga body
      await this._waitFor(() => this.world?.robot?.body, 8000)

      // Spawns de prueba (puedes cambiar posiciones)
      this.spawnEnemy(zombieBase, zombieAnims, new THREE.Vector3(2, 0, -5))
      this.spawnEnemy(zombieBase, zombieAnims, new THREE.Vector3(-3, 0, -8))
    } catch (err) {
      console.warn('No se pudieron inicializar enemigos:', err)
    }
  }

  // ⬇️ ZOMBIE: crea una instancia Enemy
  spawnEnemy(zombieBase, zombieAnims, positionVec3) {
    const model = makeZombieInstance(zombieBase)
    const enemy = new Enemy({
      scene: this.scene,
      physicsWorld: this.physics.world,
      playerRef: this.world.robot,     // Debe tener .body y .die()
      model,
      animations: zombieAnims,
  ...
  // ⬇️ ZOMBIE: util para esperar una condición
  _waitFor(checkFn, timeoutMs = 5000, intervalMs = 50) {
  ...
  destroy() {
    ...
    // ⬇️ ZOMBIE: limpiar enemigos si quedara alguno
  ...
  resetGameToFirstLevel() {
    ...
    // 💀 Destruir enemigo previo si existe (si además manejas enemigos en World)
